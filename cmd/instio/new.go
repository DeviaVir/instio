...
func newProjectInDir(c *cli.Context, path string) error {
	...
	if os.IsNotExist(err) {
		fmt.Println(fmt.Sprintf("Using '%s' as project directory", path))
		...
		case "y", "yes":
			...
			return createProjectInDir(c, path)
...
func createProjectInDir(c *cli.Context, path string) error {
	...
	if c.Bool("dev") {
		if c.String("fork") != "" {
			local = filepath.Join(gopath, "src", c.String("fork"))
		}

		err = execAndWait("git", "clone", local, "--branch", "dev", "--single-branch", path)
		if err != nil {
			return err
		}

		err = corePackages(path)
		if err != nil {
			return err
		}

		fmt.Println("Dev build cloned from " + local + ":dev")
		return nil
	}
	...
	fmt.Println("New instio project created at", path)
	return nil
}
