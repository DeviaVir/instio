				...
				if (uploadSrc.length > 0) {
					...
					switch (ext) {
						...
						case '.webm':
							...
							break;
						default:
							$(img).attr('src', '/admin/static/dashboard/img/instio-file.png');
							$(unknown)
								.css({
									position: 'absolute', 
									top: '10px', 
...
func Checkbox(fieldName string, p interface{}, attrs, options map[string]string) []byte {
	...
	checked := strings.Split(checkedVals, "__instio")
...
func Tags(fieldName string, p interface{}, attrs map[string]string) []byte {
	...
	var tags []string
	if strings.Contains(values, "__instio") {
		tags = strings.Split(values, "__instio")
	}

	// case where there is only one tag stored, thus has no separator
	if len(values) > 0 && !strings.Contains(values, "__instio") {
		tags = append(tags, values)
	}

	html := `
	<div class="col s12 __instio-tags ` + name + `">
	...
	for _, tag := range tags {
		...
		html += `<input type="hidden" class="__instio-tag ` + tag + `" name=` + tagName + ` value="` + tag + `"/>`
	...
	<script>
		$(function() {
			...
			chips.on('chip.add', function(e, chip) {
				...
				var input = $('<input>');
				input.attr({
					class: '__instio-tag '+chip.tag.split(' ').join('__'),
			...
			chips.on('chip.delete', function(e, chip) {
				...
				var sel = '.__instio-tag.' + chip.tag.split(' ').join('__');
