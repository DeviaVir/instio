...
type Meta struct {
	InstioAddonName       string `json:"addon_name"`
	InstioAddonAuthor     string `json:"addon_author"`
	InstioAddonAuthorURL  string `json:"addon_author_url"`
	InstioAddonVersion    string `json:"addon_version"`
	InstioAddonReverseDNS string `json:"addon_reverse_dns"`
	InstioAddonStatus     string `json:"addon_status"`
}
...
func Register(m Meta, fn func() interface{}) Addon {
	...
	if m.InstioAddonReverseDNS == "" {
		revDNS, err := reverseDNS(m)
		if err != nil {
			panic(err)
		}

		m.InstioAddonReverseDNS = revDNS
	}

	Types[m.InstioAddonReverseDNS] = fn
...
func register(a Addon) error {
	if a.InstioAddonName == "" {
		return fmt.Errorf(`Addon must have valid Meta struct embedded: missing %s field.`, "InstioAddonName")
	}
	if a.InstioAddonAuthor == "" {
		return fmt.Errorf(`Addon must have valid Meta struct embedded: missing %s field.`, "InstioAddonAuthor")
	}
	if a.InstioAddonAuthorURL == "" {
		return fmt.Errorf(`Addon must have valid Meta struct embedded: missing %s field.`, "InstioAddonAuthorURL")
	}
	if a.InstioAddonVersion == "" {
		return fmt.Errorf(`Addon must have valid Meta struct embedded: missing %s field.`, "InstioAddonVersion")
	}

	if _, ok := Types[a.InstioAddonReverseDNS]; !ok {
		return fmt.Errorf(`Addon "%s" has no record in the addons.Types map`, a.InstioAddonName)
	}

	// check if addon is already registered in db as addon_reverse_dns
	if db.AddonExists(a.InstioAddonReverseDNS) {
		return nil
	}
	...
	vals.Set("addon_name", a.InstioAddonName)
	vals.Set("addon_author", a.InstioAddonAuthor)
	vals.Set("addon_author_url", a.InstioAddonAuthorURL)
	vals.Set("addon_version", a.InstioAddonVersion)
	vals.Set("addon_reverse_dns", a.InstioAddonReverseDNS)
	...
	kind, ok := Types[a.InstioAddonReverseDNS]
	if !ok {
		return fmt.Errorf("Error: no addon to set with id: %s", a.InstioAddonReverseDNS)
	}
...
func reverseDNS(meta Meta) (string, error) {
	u, err := url.Parse(meta.InstioAddonAuthorURL)
	if err != nil {
		return "", nil
	}

	if u.Host == "" {
		return "", fmt.Errorf(`Error parsing Addon Author URL: %s. Ensure URL is formatted as "scheme://hostname/path?query" (path & query optional)`, meta.InstioAddonAuthorURL)
	}

	name := strings.Replace(meta.InstioAddonName, " ", "", -1)
