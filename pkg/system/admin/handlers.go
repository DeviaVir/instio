...
import (
	...
	"github.com/DeviaVir/instio/pkg/management/format"
	"github.com/DeviaVir/instio/pkg/management/manager"
	"github.com/DeviaVir/instio/pkg/system/addon"
	"github.com/DeviaVir/instio/pkg/system/admin/config"
	"github.com/DeviaVir/instio/pkg/system/admin/upload"
	"github.com/DeviaVir/instio/pkg/system/admin/user"
	...
	"github.com/DeviaVir/instio/pkg/system/search"
...
If you did not make this request, ignore this message and your password 
will remain as-is.
...
`, email, domain, key, domain)
		...
		msg := emailer.Message{
			To:      email,
			From:    fmt.Sprintf("instio@%s", domain),
...
func uploadContentsHandler(res http.ResponseWriter, req *http.Request) {
					...
					<div class="col s8">
						<div class="row">
							...
							<div class="col s5 input-field inline">
								<select class="browser-default __instio sort-order">
							...
							<script>
								$(function() {
									var sort = $('select.__instio.sort-order');
	...
	<script>
		$(function() {
			var del = $('.quick-delete-post.__instio span');
			del.on('click', function(e) {
				if (confirm("[Instio] Please confirm:\n\nAre you sure you want to delete this post?\nThis cannot be undone.")) {
...
func adminPostListItem(e editor.Editable, typeName, status string) []byte {
	...
	post := `
			<li class="col s12">
				...
				<form enctype="multipart/form-data" class="quick-delete-post __instio right" action="` + action + `" method="post">
					<span>Delete</span>
	...
	a := `
			<li class="col s12">
				<div class="row">
					...
					<div class="col s3">					
						<form enctype="multipart/form-data" class="quick-` + strings.ToLower(action) + `-addon __instio right" action="/admin/addons" method="post">
